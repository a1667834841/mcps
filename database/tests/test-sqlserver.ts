/**
 * Test script for SQL Server provider
 */
...
async function test() {
  console.log('=== SQL Server Provider Test ===\n');
  ...
  try {
    ...
    console.log('3. Listing tables in master database...');
    const tables = await provider.listTables('master');
    ...
    console.log('4. Describing table from master database...');
    try {
      if (tables.length > 0) {
        const schema = await provider.describeTable('master', tables[0].name);
    ...
    console.log('   SQL Server Version:', version.rows);
    console.log();
